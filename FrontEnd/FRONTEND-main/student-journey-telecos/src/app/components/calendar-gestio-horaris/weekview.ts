...
@Component({
            ...
            <ion-slide class="slide-container">
                ...
                <div *ngIf="0===currentViewIndex">
                    <div class="weekview-allday-table">
                        <div class="weekview-allday-label"></div>
                        <div class="weekview-allday-content-wrapper">
                            <table class="table table-fixed weekview-allday-content-table">
                                <tbody>
                                <tr>
                                    <td *ngFor="let day of views[0].dates" class="calendar-cell" [ngClass]="isWeekend(day.date) ? 'weekend-header' : ''">
                    ...
                    <init-position-scroll class="weekview-normal-event-container" [initPosition]="initScrollPosition"
                        ...
                        <table class="table table-bordered table-fixed weekview-normal-event-table">
                            ...
                            <tr *ngFor="let row of views[0].rows; let i = index">
                                <td class="calendar-hour-column calendar-hour-column-hora text-center">
                                    <span class='etiqueta-hora' id="{{isCurrentHour(hourColumnLabels[i]) ? 'current-hour' : ''}}">{{hourColumnLabels[i]}}</span>
                                </td>
                                <td *ngFor="let tm of row" class="calendar-cell calendar-event-cell" style="border: 1px solid #ddd !important;" tappable
                                    (click)="select(tm.time, tm.events)" [ngClass]="isWeekend(tm.time) ? 'weekend-cell' : ''">
                    ...
                    <init-position-scroll class="weekview-normal-event-container" [initPosition]="initScrollPosition">
                        <table class="table table-bordered table-fixed weekview-normal-event-table">
                            ...
                            <tr *ngFor="let row of views[0].rows; let i = index">
                                ...
                                <td *ngFor="let tm of row" class="calendar-cell calendar-event-cell" style="border: 1px solid #ddd !important;" [ngClass]="isWeekend(tm.time) ? 'weekend-cell' : ''">
                    ...
                    <div class="weekview-allday-table">
                        ...
                        <div class="weekview-allday-content-wrapper scroll-content">
                            <table class="table table-fixed weekview-allday-content-table">
                                <tbody>
                                <tr>
                                    <td *ngFor="let day of views[1].dates" class="calendar-cell" [ngClass]="isWeekend(day.date) ? 'weekend-header' : ''">
                                        <ng-template [ngTemplateOutlet]="weekviewAllDayEventSectionTemplate"
                                                     [ngTemplateOutletContext]="{day:day, eventTemplate:weekviewAllDayEventTemplate}">
                                        </ng-template>
                                    </td>
                    ...
                    <div class="weekview-allday-table">
                        ...
                        <div class="weekview-allday-content-wrapper scroll-content">
                            <table class="table table-fixed weekview-allday-content-table">
                                <tbody>
                                <tr>
                                    <td *ngFor="let day of views[2].dates" class="calendar-cell" [ngClass]="isWeekend(day.date) ? 'weekend-header' : ''">
                                        <ng-template [ngTemplateOutlet]="weekviewAllDayEventSectionTemplate"
                                                     [ngTemplateOutletContext]="{day:day, eventTemplate:weekviewAllDayEventTemplate}">
                                        </ng-template>
                                    </td>
    ...
    styles: [`
        ...
        *::-webkit-scrollbar {
        ...
        .table > thead > tr > th.calendar-hour-column {
...
export class WeekViewComponent implements ICalendarComponent, OnInit, OnChanges, OnDestroy, AfterViewInit {
    ...
    isCurrentHour(label) {
        const hour = new Date();
        hour.setHours(label.replace('h', ''));
        return new Date().getHours() === hour.getHours();
    }
    ...
    ngOnInit() {
        ...
        setTimeout(() => {
            const position = document.getElementById('current-hour').offsetTop - 140;

            this.initScrollPosition = position;
        } , 100);
    }
    ...
    private getHourColumnLabels(): string[] {
        ...
        for (let hour = 0, length = this.views[0].rows.length; hour < length; hour += 1) {
            ...
            if (hour === 0 && this.views[0].rows[hour][0].time.getHours() !== this.startHour) {
                ...
                hourColumnLabels.push(this.formatHourColumnLabel(time) + 'h');
            } else {
                hourColumnLabels.push(this.formatHourColumnLabel(this.views[0].rows[hour][0].time) + 'h');
            }
        }
    ...
    getTitle(): string {
        ...
        month = (currentViewStartDate.getMonth() + (date !== 1 ? 1 : 1)) % 12,
        year = currentViewStartDate.getFullYear() + (date !== 1 && month === 0 ? 1 : 0),
        headerDate = new Date(year, month - 1, 1, 12, 0, 0, 0);
